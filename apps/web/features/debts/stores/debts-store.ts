...
interface DebtsState {
  ...
  // 액션
  addDebt: (initial?: { loanType?: string }) => void;
  ...
  devtools(
    persist(
      (set) => ({
        ...
        // 새 대출 추가. `loanType` 을 전달하면 loanName 까지 바로 세팅한다 (모달에서 사용).
        addDebt: (initial) =>
          set((state) => {
            const newId = crypto.randomUUID();
            const loanType = initial?.loanType ?? "";
            const loanName = loanType
              ? `${loanType} ${state.debts.length + 1}`
      ...
      }),
      {
        ...
        migrate: (persisted, version) => {
          ...
          if (version < 3) {
            state.debts = state.debts.map((d) => {
              const next = { ...d } as DebtsItem & { loanOwner?: unknown };
              delete next.loanOwner;
              return next;
            });
          }
          return state;
