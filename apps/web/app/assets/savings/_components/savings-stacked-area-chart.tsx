...
export function SavingsStackedAreaChart({ savings }: SavingsStackedAreaChartProps) {
  const [selectedRange, setSelectedRange] = useState<TimeRange>(TimeRange.THREE_MONTHS);

  const { data, config } = prepareSavingsStackedAreaChartData(savings, selectedRange);
  ...
  if (!hasData) {
    return (
      <div className="w-full h-[300px] flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-700 rounded-lg">
        <span className="text-gray-400 text-sm mb-2">저축 히스토리 없음</span>
        <span className="text-xs text-gray-500">잔액을 변경하면 히스토리가 생성됩니다</span>
      </div>
    );
  }
